...
- (void)viewDidLoad {
    ...
    [[ATAPI sharedInstance] startWithAppID:@"h680600650ee63" appKey:@"aafbad3cf963987d514dee38062e67b8c" error:nil];

    // 设置测试竞价的idfa，例如Meta广告测试设备等
//    [ATAPI setHeaderBiddingTestModeWithDeviceID:@"B08392CD-0E68-4682-BA30-39AB7A42F831"];

    self.titleArr= @[@"开屏广告",@"自渲染信息流",@"模版渲染Banner广告",@"模版渲染激励视频广告"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        ...
        [[ATAdManager sharedManager] loadADWithPlacementID:@"n1ge27qaotscc5"
                                                     extra:mutableDict
                                                  delegate:self
                                             containerView:_bottomView];
    }else if (indexPath.row==1){
        ToponNativeViewController * native = [[ToponNativeViewController alloc]init];
