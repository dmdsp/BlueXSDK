//
//  DomobCustomNativeAdapter.m
//  DMAd
//
//  Created by 刘士林 on 2024/8/27.
//

#import "DomobCustomNativeAdapter.h"
...
+ (void)sendWinnerNotifyWithCustomObject:(id)customObject secondPrice:(NSString*)price userInfo:(NSDictionary<NSString *, NSString *> *)userInfo{
    //customObject为当广告对象
    if ([customObject isKindOfClass:[DM_NativeAd class]]) {
        DM_NativeAd * ad = customObject;
        [ad biddingNativSuccess:price.integerValue];
    }
}
...
//+ (void)sendBackLossPrice:(DM_NativeAd *)ad code:(DMAdBiddingCode)code price:(NSInteger)price {
...
- (void)loadADWithInfo:(NSDictionary*)serverInfo localInfo:(NSDictionary*)localInfo completion:(void (^)(NSArray<NSDictionary *> *, NSError *))completion {
    ...
    if (request) { //竞价失败不会进入该方法，所以处理竞价成功的逻辑
        ...
        if (request.customObject != nil) { // load secced 且 广告数据可用(原则上是检查广告是否可用的，TM并没有提供所以使用检查是否广告对象来替代)
            ...
            if ([request.customObject isKindOfClass:[DM_NativeAd class]]) {
                DM_NativeAd * feed = request.customObject;
                [feed setValue:_customEvent forKey:@"delegate"];
                BOOL isDmTemplateAd = [request.extraInfo[@"dm_is_template_ad"] boolValue];
                [_customEvent feedRendering:feed];
            }
        ...
        // 普通瀑布流的广告配置，进行加载广告
        dispatch_async(dispatch_get_main_queue(), ^{
            self.nativeAd = [DM_NativeAd new] ;
            [self.nativeAd loadNativeAdTemplateAdWithSlotID:serverInfo[@"unitid"]  delegate:self.customEvent];
        });
    }
}
